...
def is_same_or_inside_path(child: Path | str, parent: Path | str) -> bool:
    ...
    if os.path.normcase(str(child_abs)) == os.path.normcase(str(parent_abs)):
        return True
    try:
...
def validate_cloud_relative_path(value: str) -> str:
    ...
    if not text:
        raise PathValidationError("OneDrive 中的路径不能为空。")
    if text in {".", "/"}:
        raise PathValidationError("OneDrive 中的路径无效。")
    if DRIVE_RE.match(text):
        raise PathValidationError("OneDrive 中的路径必须是相对路径。")
    if text.startswith("/") or text.startswith("\\"):
        raise PathValidationError("OneDrive 中的路径必须是相对路径。")

    pure = PurePosixPath(text)
    if any(part in {".", ".."} for part in pure.parts):
        raise PathValidationError("OneDrive 中的路径不能包含 . 或 ..。")
    if not pure.parts or pure.parts[0] != "data":
        raise PathValidationError("OneDrive 中的路径必须位于 data/ 下。")
    if len(pure.parts) < 2:
        raise PathValidationError("OneDrive 中的路径不能指向 data 根目录。")
...
def validate_local_path(
    local_path: Path | str,
    onedrive_root: Path | str,
    cloud_target_path: Path | str,
) -> Path:
    ...
    if is_same_or_inside_path(local_abs, cloud_abs):
        raise PathValidationError("不能映射当前云端目标目录或其内部目录。")
    return local_abs
