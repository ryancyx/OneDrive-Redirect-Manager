...
用户可以选择一个 OneDrive 内的文件夹作为软件工作目录，例如：
...
创建项目后，本地源路径会变成 junction 链接，真实数据保存在 OneDrive 目标目录中。
...
|---|---|---|
| 本地有数据 | 云端不存在或为空 | 迁移本地数据到云端并创建 junction |
| 本地为空 | 云端有数据 | 删除本地空入口并创建 junction |
| 本地为空 | 云端为空 | 直接创建 junction |
| 本地不存在 | 云端已存在 | 创建 junction 指向云端 |
| 本地和云端都有数据 | 进入冲突处理 |
...
当本地目录和 OneDrive 目标目录中都存在数据时，软件不会直接覆盖任何一边，而是进入冲突处理流程。

当前支持：

* 取消操作
* 备份本地文件夹并使用 OneDrive 数据

选择“备份本地文件夹并使用 OneDrive 数据”后，软件会在**本地源文件夹旁边**创建一个 `_backup` 目录保存原本的本地数据。例如：
...
随后本地源路径会被替换为指向 OneDrive 目标目录的 junction。
...
| 选择 | 结果 |
|---|---|
| 不勾选任何选项 | 只删除配置记录，不删除本地链接，也不删除 OneDrive 数据 |
| 删除 OneDrive 目标文件夹 | 删除云端真实数据，保留本地 junction |
| 删除本地链接 | 删除本地 junction 入口，保留 OneDrive 数据 |
| 两个都勾选 | 先删除本地 junction，再删除 OneDrive 目标目录，最后删除配置 |
...
恢复操作会将 OneDrive 中的数据复制回本地，并删除本地 junction，使该目录重新成为普通本地文件夹。
...
OneDrive Redirect Manager 采用轻量分层结构，将文件系统逻辑、项目配置和 QML 界面分离。

```text
OneDriveRedirectManager/
├─ run.py
├─ assets/
│  └─ OneDriveRedirectManager.ico
├─ src/
│  └─ onedrive_redirector/
│     ├─ app.py
│     ├─ core/
...
```text
设置 OneDrive 工作目录 → 新建项目 → 创建 junction → 使用原软件 → 由 OneDrive 同步数据
```

### 1. 设置 OneDrive 工作目录

在设置页面中选择一个 OneDrive 内的工作目录。

### 2. 新建项目

填写项目 ID、名称、本地源文件夹和 OneDrive 路径。
...
后续可以根据需要删除项目、保留云端数据，或恢复到本地并取消同步。
...
游戏仍然读写原路径，而真实数据由 OneDrive 保存和同步。
...
* 当前不提供跨平台同步能力
* 当前不直接管理 OneDrive 登录和同步状态
* 当前不支持任务队列和批量自动修复
* 删除 OneDrive 目标文件夹前仍需用户自行确认数据是否需要保留
* 如果 OneDrive 正在同步或文件被占用，部分操作可能需要等待
...
如果想删除 OneDrive 工作目录中的配置和数据，需要手动删除曾经选择的 OneDrive 工作目录，例如：

```text
D:\RyanC\OneDrive\应用\Saved\ODR
```

删除 OneDrive 工作目录前请确认其中数据不再需要。
...
* 项目类型：Windows 桌面工具 / OneDrive 目录重定向管理器
